<?php
session_start();
$password = 'admin123'; // ← Зміни пароль тут

if (!isset($_SESSION['admin_logged'])) {
    if (isset($_POST['password']) && $_POST['password'] === $password) {
        $_SESSION['admin_logged'] = true;
    } else {
        echo '<!DOCTYPE html><html lang="uk"><head><meta charset="UTF-8"><title>Вхід</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-gray-900 text-white flex items-center justify-center min-h-screen"><form method="post" class="bg-gray-800 p-12 rounded-3xl w-96"><h1 class="text-3xl font-bold mb-8 text-center">Вхід в адмінку</h1><input type="password" name="password" placeholder="Пароль" class="w-full bg-gray-700 px-6 py-4 rounded-2xl mb-6"><button type="submit" class="w-full bg-emerald-600 py-4 rounded-2xl font-bold">УВІЙТИ</button></form></body></html>';
        exit;
    }
}
...
<head>
    ...
    <title>Адмінка — Your site PRO v1.0</title>
...
<div class="flex h-screen overflow-hidden">
    ...
    <div id="sidebar" class="sidebar w-72 bg-gray-900 p-6 overflow-y-auto border-r border-gray-800 md:relative fixed inset-y-0 left-0 z-50">
        ...
        <nav class="space-y-1">
            <a href="#" onclick="switchTab('general')" class="tab-link flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-emerald-600 active"><i class="fa-solid fa-cog"></i> Загальні</a>
            <a href="#" onclick="switchTab('header')" class="tab-link flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-emerald-600"><i class="fa-solid fa-heading"></i> Шапка сайту</a>
            ...
            <a href="#" onclick="switchTab('services')" class="tab-link flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-emerald-600"><i class="fa-solid fa-list"></i> Послуги</a>
            <a href="#" onclick="switchTab('advantages')" class="tab-link flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-emerald-600"><i class="fa-solid fa-star"></i> Переваги</a>
            <a href="#" onclick="switchTab('how')" class="tab-link flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-emerald-600"><i class="fa-solid fa-clock"></i> Як ми працюємо</a>
            <a href="#" onclick="switchTab('gallery')" class="tab-link flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-emerald-600"><i class="fa-solid fa-images"></i> Галерея</a>
            <a href="#" onclick="switchTab('reviews')" class="tab-link flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-emerald-600"><i class="fa-solid fa-comment-dots"></i> Відгуки</a>
            <a href="#" onclick="switchTab('seo_text')" class="tab-link flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-emerald-600"><i class="fa-solid fa-text-height"></i> SEO текст</a>
            ...
            <a href="#" onclick="switchTab('order_form')" class="tab-link flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-emerald-600"><i class="fa-solid fa-envelope"></i> Форма замовлення</a>
			...
			<a href="#" onclick="switchTab('email')" class="tab-link flex items-center gap-3 px-5 py-4 rounded-2xl hover:bg-emerald-600"><i class="fa-solid fa-envelope"></i> Налаштування Email</a>
        <center><a href="?logout=1" onclick="return confirm('Вийти з адмінки?')" class=" md:flex items-center gap-2 bg-red-600 hover:bg-red-700 px-5 py-2.5 rounded-2xl text-sm font-medium">
    ...
    <div class="flex-1 overflow-y-auto pt-16 md:pt-0 p-6 md:p-10">
        ...
        <form method="POST" id="adminForm">
            ...
            <button type="submit" name="save" onclick="prepareJSON()" class="fixed bottom-6 right-6 md:bottom-8 md:right-8 bg-emerald-600 hover:bg-emerald-700 px-10 py-5 rounded-3xl text-lg font-bold shadow-2xl flex items-center gap-3 z-50">
                <i class="fa-solid fa-save"></i> ЗБЕРЕГТИ ВСІ ЗМІНИ
            </button>

            <!-- TAB GENERAL -->
            <div id="tab-general" class="tab-content active">
                <h2 class="text-4xl font-bold mb-8">Загальні налаштування</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div><label class="block text-sm mb-2">Телефон</label><input type="text" class="w-full bg-gray-800 px-6 py-4 rounded-2xl" value="<?= htmlspecialchars($data['general']['phone']) ?>" onchange="updateData('general.phone', this.value)"></div>
                    ...
                    <div><label class="block text-sm mb-2">Адреса</label><input type="text" class="w-full bg-gray-800 px-6 py-4 rounded-2xl" value="<?= htmlspecialchars($data['general']['address']) ?>" onchange="updateData('general.address', this.value)"></div>
                    <div><label class="block text-sm mb-2">Назва сайту (footer)</label><input type="text" class="w-full bg-gray-800 px-6 py-4 rounded-2xl" value="<?= htmlspecialchars($data['general']['site_name']) ?>" onchange="updateData('general.site_name', this.value)"></div>
                    <div><label class="block text-sm mb-2">Рік</label><input type="text" class="w-full bg-gray-800 px-6 py-4 rounded-2xl" value="<?= htmlspecialchars($data['general']['year']) ?>" onchange="updateData('general.year', this.value)"></div>
                </div>
            </div>

            <!-- TAB HEADER (нова вкладка) -->
            <div id="tab-header" class="tab-content">
                <h2 class="text-4xl font-bold mb-8">Шапка сайту</h2>
                <div class="space-y-8">
                    <div>
                        <label class="block text-sm mb-2">Текст логотипу (CleanPro)</label>
                    ...
                    <div>
                        <h3 class="text-xl font-semibold mb-4 flex items-center gap-2"><i class="fa-solid fa-list"></i> Пункти навігації</h3>
                        <div id="nav-container" class="space-y-6"></div>
                        <button type="button" onclick="addNavItem()" 
                                class="mt-6 bg-emerald-600 hover:bg-emerald-700 px-8 py-4 rounded-3xl flex items-center gap-2">
                            <i class="fa-solid fa-plus"></i> Додати пункт меню
                        </button>
                    </div>
                </div>
            </div>
            ...
            <!-- TAB HERO (залишив без змін, для повноти) -->
            <div id="tab-hero" class="tab-content">
                <h2 class="text-4xl font-bold mb-8">Hero секція</h2>
                <div class="space-y-6">
                    ...
                    <div><label class="block text-sm mb-2">Текст</label><textarea class="w-full bg-gray-800 px-6 py-4 rounded-2xl h-32" onchange="updateData('hero.p', this.value)"><?= htmlspecialchars($data['hero']['p']) ?></textarea></div>
                    <div class="grid grid-cols-2 gap-6">
                        <div><label class="block text-sm mb-2">Кнопка "Замовити"</label><input type="text" class="w-full bg-gray-800 px-6 py-4 rounded-2xl" value="<?= htmlspecialchars($data['hero']['button_order']) ?>" onchange="updateData('hero.button_order', this.value)"></div>
                        <div><label class="block text-sm mb-2">Кнопка телефон</label><input type="text" class="w-full bg-gray-800 px-6 py-4 rounded-2xl" value="<?= htmlspecialchars($data['hero']['button_phone']) ?>" onchange="updateData('hero.button_phone', this.value)"></div>
                    </div>
                </div>
            </div>

            <!-- TAB SERVICES (залишив без змін) -->
            <div id="tab-services" class="tab-content">
                <h2 class="text-4xl font-bold mb-8">Послуги</h2>
                <div id="services-container" class="space-y-8"></div>
                <button type="button" onclick="addService()" class="mt-6 bg-emerald-600 px-8 py-4 rounded-3xl flex items-center gap-2"><i class="fa-solid fa-plus"></i> Додати послугу</button>
            </div>

            <!-- TAB ADVANTAGES -->
            <div id="tab-advantages" class="tab-content">
                <h2 class="text-4xl font-bold mb-8">Переваги</h2>
                <div id="advantages-container" class="space-y-8"></div>
                <button type="button" onclick="addAdvantage()" class="mt-6 bg-emerald-600 px-8 py-4 rounded-3xl flex items-center gap-2"><i class="fa-solid fa-plus"></i> Додати перевагу</button>
            </div>

            <!-- TAB HOW -->
            <div id="tab-how" class="tab-content">
                <h2 class="text-4xl font-bold mb-8">Як ми працюємо</h2>
                <div id="how-container" class="space-y-8"></div>
                <button type="button" onclick="addHowStep()" class="mt-6 bg-emerald-600 px-8 py-4 rounded-3xl flex items-center gap-2"><i class="fa-solid fa-plus"></i> Додати крок</button>
            </div>

            <!-- TAB GALLERY -->
            <div id="tab-gallery" class="tab-content">
                <h2 class="text-4xl font-bold mb-8">Галерея</h2>
                <div id="gallery-container" class="space-y-8"></div>
                <button type="button" onclick="addGalleryImage()" class="mt-6 bg-emerald-600 px-8 py-4 rounded-3xl flex items-center gap-2"><i class="fa-solid fa-plus"></i> Додати фото</button>
            </div>

            <!-- TAB REVIEWS -->
            <div id="tab-reviews" class="tab-content">
                <h2 class="text-4xl font-bold mb-8">Відгуки</h2>
                <div id="reviews-container" class="space-y-8"></div>
                <button type="button" onclick="addReview()" class="mt-6 bg-emerald-600 px-8 py-4 rounded-3xl flex items-center gap-2"><i class="fa-solid fa-plus"></i> Додати відгук</button>
            </div>

            <!-- TAB SEO TEXT -->
            <div id="tab-seo_text" class="tab-content">
                <h2 class="text-4xl font-bold mb-8">SEO текст</h2>
                <div class="space-y-6">
                    ...
                    <div><label class="block text-sm mb-2">Параграф 1</label><textarea class="w-full bg-gray-800 px-6 py-4 rounded-2xl h-32" onchange="updateData('seo_text.p1', this.value)"><?= htmlspecialchars($data['seo_text']['p1']) ?></textarea></div>
                    <div><label class="block text-sm mb-2">Параграф 2</label><textarea class="w-full bg-gray-800 px-6 py-4 rounded-2xl h-32" onchange="updateData('seo_text.p2', this.value)"><?= htmlspecialchars($data['seo_text']['p2']) ?></textarea></div>
                </div>
            </div>
            ...
            <div id="tab-faq" class="tab-content">
                ...
                <button type="button" onclick="addFaq()" class="mt-6 bg-emerald-600 px-8 py-4 rounded-3xl flex items-center gap-2"><i class="fa-solid fa-plus"></i> Додати питання</button>
            </div>

            <!-- TAB ORDER FORM -->
            <div id="tab-order_form" class="tab-content">
                <h2 class="text-4xl font-bold mb-8">Форма замовлення</h2>
                <div class="space-y-6">
                    ...
                    <div><label class="block text-sm mb-2">Підзаголовок</label><input type="text" class="w-full bg-gray-800 px-6 py-4 rounded-2xl" value="<?= htmlspecialchars($data['order_form']['p']) ?>" onchange="updateData('order_form.p', this.value)"></div>
            ...
            <div id="tab-footer" class="tab-content">
                ...
                <div class="space-y-6">
                    ...
                    <div><label class="block text-sm mb-2">Райони</label><textarea class="w-full bg-gray-800 px-6 py-4 rounded-2xl h-32" onchange="updateData('footer.districts', this.value)"><?= htmlspecialchars($data['footer']['districts']) ?></textarea></div>
            ...
            <div id="tab-custom" class="tab-content">
                ...
                <div class="space-y-8">
                    <div>
                        <label class="block text-sm mb-2">Код у &lt;head&gt;</label>
                    ...
                    <div>
                        <label class="block text-sm mb-2">Додатковий CSS</label>
                    ...
                    <div>
                        <label class="block text-sm mb-2">Додатковий JavaScript</label>
                    ...
                    <div>
                        <label class="block text-sm mb-2">HTML перед &lt;/body&gt;</label>
			...
			<!-- TAB EMAIL SETTINGS (українською) -->
<div id="tab-email" class="tab-content">
    <h2 class="text-4xl font-bold mb-8">Налаштування Email</h2>
    <div class="space-y-6 max-w-2xl">
        <div>
            <label class="block text-sm mb-2 font-medium">Отримувач листа (to)</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label class="block text-sm mb-2 font-medium">Відправник (ім’я)</label>
                <input type="text" class="w-full bg-gray-800 px-6 py-4 rounded-2xl" 
    ...
    globalData.header.nav_items.forEach((item, i) => {
        ...
        div.innerHTML = `
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs text-gray-400 mb-1">Текст</label>
                    <input type="text" value="${item.text||''}" placeholder="Послуги" 
                           class="bg-gray-900 px-5 py-3 rounded-2xl w-full" 
                           onchange="globalData.header.nav_items[${i}].text=this.value">
                </div>
                <div>
                    <label class="block text-xs text-gray-400 mb-1">Якір (#services)</label>
            ...
            <button onclick="removeNav(${i})" class="mt-4 text-red-500 hover:text-red-600 flex items-center gap-1">
                <i class="fa-solid fa-trash"></i> Видалити пункт
            </button>
        `;
        c.appendChild(div);
    });
...
function renderServices() {
    ...
    globalData.services.items.forEach((item, i) => {
        ...
        div.className = 'bg-gray-800 p-6 rounded-3xl';
        div.innerHTML = `
            <input type="text" value="${item.image||''}" placeholder="Зображення URL" class="bg-gray-900 px-5 py-3 rounded-2xl w-full" onchange="globalData.services.items[${i}].image=this.value">
            <input type="text" value="${item.title||''}" placeholder="Заголовок" class="bg-gray-900 px-5 py-3 rounded-2xl w-full mt-3" onchange="globalData.services.items[${i}].title=this.value">
            ...
            <input type="text" value="${item.footer||''}" placeholder="Нижній текст" class="bg-gray-900 px-5 py-3 rounded-2xl w-full mt-3" onchange="globalData.services.items[${i}].footer=this.value">
            <button onclick="removeService(${i})" class="mt-4 text-red-500">Видалити</button>
        `;
        c.appendChild(div);
    });
}
...
function renderAdvantages() {
    ...
    globalData.advantages.items.forEach((item, i) => {
        ...
        div.innerHTML = `
            ...
            <input type="text" value="${item.title||''}" placeholder="Заголовок" class="bg-gray-900 px-5 py-3 rounded-2xl w-full mt-3" onchange="globalData.advantages.items[${i}].title=this.value">
            ...
            <button onclick="removeAdvantage(${i})" class="mt-4 text-red-500">Видалити</button>
        `;
        c.appendChild(div);
    });
}
...
function renderHow() {
    ...
    globalData.how.steps.forEach((item, i) => {
        ...
        div.className = 'bg-gray-800 p-6 rounded-3xl';
        div.innerHTML = `
            <input type="text" value="${item.num||''}" placeholder="Номер" class="bg-gray-900 px-5 py-3 rounded-2xl w-16" onchange="globalData.how.steps[${i}].num=this.value">
            <input type="text" value="${item.title||''}" placeholder="Заголовок" class="bg-gray-900 px-5 py-3 rounded-2xl w-full mt-3" onchange="globalData.how.steps[${i}].title=this.value">
            ...
            <button onclick="removeHow(${i})" class="mt-4 text-red-500">Видалити</button>
        `;
        c.appendChild(div);
    });
}
...
function renderGallery() {
    ...
    globalData.gallery.images.forEach((item, i) => {
        ...
        div.innerHTML = `
            ...
            <button onclick="removeGallery(${i})" class="mt-4 text-red-500">Видалити</button>
        `;
        c.appendChild(div);
    });
}
...
function renderReviews() {
    ...
    globalData.reviews.items.forEach((item, i) => {
        ...
        div.className = 'bg-gray-800 p-6 rounded-3xl';
        div.innerHTML = `
            <input type="text" value="${item.stars||''}" placeholder="Зірки" class="bg-gray-900 px-5 py-3 rounded-2xl w-full" onchange="globalData.reviews.items[${i}].stars=this.value">
            ...
            <input type="text" value="${item.name||''}" placeholder="Ім'я" class="bg-gray-900 px-5 py-3 rounded-2xl w-full mt-3" onchange="globalData.reviews.items[${i}].name=this.value">
            <input type="text" value="${item.info||''}" placeholder="Інфо" class="bg-gray-900 px-5 py-3 rounded-2xl w-full mt-3" onchange="globalData.reviews.items[${i}].info=this.value">
            <button onclick="removeReview(${i})" class="mt-4 text-red-500">Видалити</button>
        `;
        c.appendChild(div);
    });
}
...
function renderFaq() {
    ...
    globalData.faq.items.forEach((item, i) => {
        ...
        div.className = 'bg-gray-800 p-6 rounded-3xl';
        div.innerHTML = `
            <input type="text" value="${item.q||''}" placeholder="Питання" class="bg-gray-900 px-5 py-3 rounded-2xl w-full" onchange="globalData.faq.items[${i}].q=this.value">
            ...
            <button onclick="removeFaq(${i})" class="mt-4 text-red-500">Видалити</button>
        `;
        c.appendChild(div);
    });
}
