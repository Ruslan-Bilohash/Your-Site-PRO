<?php
// ====================== index.php ======================
// Main Landing Page — Your Site PRO v1.0 (ENGLISH VERSION)
// Author: Ruslan Bilohash | bilohash.com | 2026

$json = json_decode(file_get_contents('content.json'), true);
$success = isset($_GET['success']) && $_GET['success'] == 1;
$error = isset($_GET['error']) && $_GET['error'] == 1 ? ($json['order_form']['error_text'] ?? 'Something went wrong. Please try again.') : '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?= htmlspecialchars($json['meta']['title'] ?? 'Professional Cleaning Services') ?></title>
    ...
    <header class="bg-white shadow-lg sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div><span class="text-3xl font-bold tracking-tight"><?= htmlspecialchars($json['header']['logo_text'] ?? 'Your Site') ?></span></div>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-6 text-center">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="#order" class="bg-emerald-600 hover:bg-emerald-700 text-white text-xl font-semibold px-10 py-6 rounded-3xl inline-flex items-center gap-3 transition shadow-xl">
                    <i class="fa-solid fa-broom"></i> <?= htmlspecialchars($json['hero']['button_order'] ?? 'Order Now') ?>
                </a>
        ...
        <div class="max-w-4xl mx-auto px-6">
            ...
            <?php if ($success): ?>
                <div class="bg-emerald-500 text-white p-8 rounded-3xl text-center mb-8">
                    ...
                    <p class="mt-3"><?= htmlspecialchars($json['order_form']['success_text'] ?? 'We will contact you soon.') ?></p>
                </div>
            <?php endif; ?>
            <?php if ($error): ?>
            ...
            <form method="POST" action="submit.php" class="grid md:grid-cols-2 gap-8">
                <div>
                    <label class="block text-sm mb-2"><?= htmlspecialchars($json['order_form']['labels']['name'] ?? 'Name') ?></label>
                ...
                <div>
                    <label class="block text-sm mb-2"><?= htmlspecialchars($json['order_form']['labels']['phone'] ?? 'Phone') ?></label>
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm mb-2"><?= htmlspecialchars($json['order_form']['labels']['message'] ?? 'Address & Preferences') ?></label>
        ...
        <div class="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-12">
            <div>
                <div class="flex items-center gap-3 text-3xl mb-6">
                    ...
                    <span class="font-bold"><?= htmlspecialchars($json['general']['site_name'] ?? 'Your Site') ?></span>
                </div>
        ...
        <div class="mt-16 pt-8 border-t border-gray-800 text-center text-sm text-gray-500">
            ...
            <div class="mt-6">
                <a href="donate.php" class="inline-flex items-center gap-2 text-emerald-400 hover:text-emerald-300 transition text-sm">
                    <i class="fa-solid fa-heart"></i> Support the Author
                </a>
            </div>
